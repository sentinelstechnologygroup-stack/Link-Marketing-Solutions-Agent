...
- Do not add new legacy provider dependencies, SDK calls, environment variables, or deployment workflows.
- Do not remove existing legacy provider-backed behavior until an equivalent implementation exists and its parity is recorded.
...
- `legacy provider/entities/`: legacy entity definitions to preserve as migration references until parity is complete.
- `legacy provider/functions/`: legacy backend functions to inventory and replace server-side.
...
- Do not use legacy provider CLI workflows for new development or deployment.
