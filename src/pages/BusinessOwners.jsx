  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-heading font-semibold tracking-tight">Business Owners & Realtors</h1><p className="text-muted-foreground text-sm mt-1">Receiving parties for warm transfers and appointments</p></div>
...
function OwnerDialog({ brands, onClose, onSaved }) {
  ...
  return (
    <Dialog open onOpenChange={onClose}><DialogContent className="max-w-lg">
      <DialogHeader><DialogTitle>Add Business Owner / Realtor</DialogTitle></DialogHeader>
      <form onSubmit={submit} className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div><Label>Role type</Label><select className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm" value={form.role_type} onChange={e => setForm({...form, role_type: e.target.value})}><option value="business_owner">Business owner</option><option value="realtor">Realtor</option><option value="licensed_professional">Licensed professional</option></select></div>
          <div><Label>License state</Label><Input value={form.license_state} onChange={e => setForm({...form, license_state: e.target.value})} /></div>
        </div>
